  ...
  return (
    <main className="flex flex-col gap-16 py-24">
      <div className="mx-auto flex w-full max-w-[1400px] flex-col gap-16 px-6 md:px-10">
        ...
        <div className="relative aspect-video w-full overflow-hidden rounded-2xl border border-[var(--card-border)]">
          {imageExists ? (
          ...
          <div className="absolute inset-x-0 bottom-0 flex justify-start p-0">
            <div className="rounded-tr-2xl bg-[rgba(10,10,11,0.75)] px-6 py-6 backdrop-blur-sm sm:px-8 sm:py-8">
              <span className="block font-sans text-5xl font-semibold leading-none text-foreground md:text-[72px]">
                {item.metricHighlight}
              </span>
              <span className="mt-3 block font-mono text-[10px] uppercase tracking-[0.2em] text-zinc-400">
                {item.metricLabel}
              </span>
            </div>
          </div>
        ...
        <div className="mx-auto flex w-full max-w-[800px] flex-col">
          {STORY_BLOCKS.map((block, index) => (
            <div
              key={block.key}
              className={`flex flex-col gap-4 py-12 ${index > 0 ? "border-t border-white/8" : ""}`}
            >
              <span className="font-mono text-[10px] uppercase tracking-[0.22em] text-accent">
                {block.eyebrow}
              </span>
              <h2 className="text-2xl font-semibold text-foreground">{block.heading}</h2>
              <p className="text-[17px] leading-relaxed text-zinc-400">{item[block.key]}</p>
            </div>
          ))}
        </div>

        {/* Metadata strip */}
        <dl className="grid grid-cols-2 gap-8 border-y border-white/8 py-8 md:grid-cols-4">
          <div>
            <dt className="font-mono text-[9px] uppercase tracking-[0.22em] text-zinc-500">
              Sector
            </dt>
        ...
        {/* Prev / next */}
        <nav aria-label="Otros casos" className="grid gap-8 border-t border-white/8 pt-12 sm:grid-cols-2">
