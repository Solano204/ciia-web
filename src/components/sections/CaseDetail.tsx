      ...
      <div className="mx-auto flex w-full max-w-[1400px] flex-col gap-16 px-6 md:px-10">
        ...
        <div className="relative aspect-video w-full overflow-hidden rounded-2xl border border-[var(--border-v2)]">
          ...
          <div className="absolute inset-x-0 bottom-0 flex justify-start p-0">
            <div className="glass-3-v2 px-6 py-6 sm:px-8 sm:py-8">
              ...
              <span className="mt-3 block font-mono text-[10px] uppercase tracking-[0.2em] text-[var(--text-secondary-v2)]">
                {item.metricLabel}
              </span>
            </div>
          </div>
        ...
        <div className="mx-auto flex w-full max-w-[800px] flex-col">
          {STORY_BLOCKS.map((block, index) => (
            <div
              key={block.key}
              className={`flex flex-col gap-4 py-12 ${index > 0 ? "border-t border-[var(--border-v2)]" : ""}`}
            >
              ...
              <p className="text-[17px] leading-relaxed text-[var(--text-secondary-v2)]">{item[block.key]}</p>
            </div>
          ))}
        </div>

        {/* Metadata strip */}
        <dl className="grid grid-cols-2 gap-8 border-y border-[var(--border-v2)] py-8 md:grid-cols-4">
          <div>
            <dt className="font-mono text-[9px] uppercase tracking-[0.22em] text-[var(--text-muted-v2)]">
              Sector
            </dt>
        ...
        {/* Prev / next */}
        <nav aria-label="Otros casos" className="grid gap-8 border-t border-[var(--border-v2)] pt-12 sm:grid-cols-2">
