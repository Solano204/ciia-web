...
void main() {
  ...
  vec3 objectNormal = getNormal(position);
...
void main() {
  ...
  vec3 color = diffuse * (0.2 + diff * 0.8) + lightColor * spec * 0.8;
  ...
  if (uLightMode > 0.5) {
    ...
    color = mix(vec3(1.0), chroma, clamp(energy * 0.98, 0.0, 0.94));
  }
...
function createBeamMaterial(cfg: {
  ...
  return new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      diffuse: { value: new THREE.Color(...hexToNormalizedRGB(cfg.beamColor)) },
      lightColor: { value: new THREE.Color(...hexToNormalizedRGB(cfg.lightColor)) },
      lightDirection: { value: new THREE.Vector3(0, 3, 10).normalize() },
    },
...
const Beams: FC<BeamsProps> = ({
  ...
  const beamMaterial = useMemo(
    () => createBeamMaterial({ beamColor, lightColor, speed, noiseIntensity, scale, lightMode }),
    [beamColor, lightColor, speed, noiseIntensity, scale, lightMode],
  );
