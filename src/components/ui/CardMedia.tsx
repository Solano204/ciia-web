...
export function CardMedia({ media, aspect }: { media: CardMediaData; aspect: MediaAspect }) {
  switch (media.kind) {
    ...
    case "text":
      return (
        <p className="font-sans text-xs leading-relaxed text-[var(--text-secondary)]">{media.content}</p>
      );
    case "stat":
      return (
        <div className="flex flex-col gap-1">
          <span className="font-display text-4xl font-semibold text-foreground">{media.value}</span>
