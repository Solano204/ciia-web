...
export function EyebrowBadge({ children, className = "", tone = "amber" }: Props) {
  if (tone === "muted-v2") {
    return (
      <span
        className={`inline-flex items-center gap-2 rounded-full border border-[var(--line-strong)] bg-white/[0.04] px-3 py-1.5 font-sans text-[12px] font-medium uppercase tracking-[0.08em] text-muted backdrop-blur-md ${className}`}
      >
  ...
  return (
    <span
      className={`inline-flex items-center gap-2 rounded-full border border-white/12 bg-white/[0.04] px-3 py-1.5 font-sans text-[12px] font-medium uppercase tracking-[0.08em] text-accent backdrop-blur-md ${className}`}
      style={{
        boxShadow:
