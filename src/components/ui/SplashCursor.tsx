...
// Stable module-level default: an inline object literal as a default *param*
// value would be a fresh reference every render, which — sitting in the
// effect's dependency array — would tear down and reinit all of WebGL on
// every unrelated re-render of a caller that doesn't pass BACK_COLOR.
...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvas: HTMLCanvasElement) {
      ...
      // Bail quietly instead of throwing — a cursor effect failing to
      // acquire a context must never take the page down with it.
      if (!gl) return null;
    ...
    let blitElementBuffer: WebGLBuffer | null = null;
    ...
    function resizeFBO(
      ...
      blit(newFBO, false);
      // The old FBO's contents were just copied forward into newFBO — safe
      // to free now that the blit has read from it.
      deleteFBO(target);
      return newFBO;
    }
    ...
    function initFramebuffers() {
      ...
      // Recreated unconditionally on every resize — dispose the previous
      // pair first instead of leaking them.
    ...
    function startLoop() {
      if (animationFrameId === null && isTabVisible) {
        animationFrameId = requestAnimationFrame(updateFrame);
      }
    }
    ...
    return () => {
      ...
      Object.values(displayMaterial.programs).forEach((program) => {
      ...
      if (blitElementBuffer) gl.deleteBuffer(blitElementBuffer);
    };
  }, [
    active,
    SIM_RESOLUTION,
